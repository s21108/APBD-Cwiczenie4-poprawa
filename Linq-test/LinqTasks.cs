...
namespace Linq_test
{
    public static class LinqTasks
    {
        ...
        {
            return Emps.Where(x => x.Salary == Task3());
        }
        ...
        {
            return from e in Emps select new { Nazwisko = e.Ename, Praca = e.Job};
        }
        ...
        {
            var query = from e in Emps join d in Depts on e.Deptno equals d.Deptno select new {Ename = e.Ename, Job = e.Job, Dname = d.Dname};
        ...
        {
            return Emps.GroupBy(x => x.Job).Select(group => new { Job = group.Key, Count = group.Count() }).OrderBy(x => x.Job);
        }
        ...
        {
            var query = Emps.Any(x => x.Job == "Backend programmer");
            return query;
        }
        ...
        {
            return Emps.OrderByDescending(x => x.HireDate).Where(x => x.Job == "Frontend programmer").FirstOrDefault();
        }
